...
buoy <- read_table("raw-data/buoy_2024.txt")
head(buoy, 7)
...
buoy <- read_table("raw-data/buoy_2024.txt", comment = "#")
head(buoy, 7)
...
buoy_update <- write_csv(buoy, file = "raw-data/buoy_update.csv")
```
...
noaa_buoy <- read_csv("raw-data/buoy_update.csv", col_types = cols(.default = "c"), na = c("99", "999")) #https://github.com/tidyverse/readr/issues/148
...
```{r, eval = FALSE}
#| label: parse-double
buoy_final <- noaa_buoy |> 
  mutate(across(.cols = everything(), ~parse_double(.x)))

head(buoy_final)

write_csv(buoy_final, file = "clean-data/buoy_final.csv")
```

```{r}
buoy_final <- read_csv("clean-data/buoy_final.csv")
```
...
buoy_final |> 
  ggplot() +
  geom_jitter(aes(x = factor(mm), y = atmp, color = factor(mm)), width = 0.1, alpha = 0.1) +
  geom_smooth(aes(x = mm, y = atmp), color = "black") +
  scale_color_viridis_d() +
  guides(color = "none") +
  theme_light() +
  labs(x = "Month in 2024", y = "Air Temperature (Celsius)", title = "Monterey Bay Caynon Air Temperature on 2024", caption = "Source: NOAA National Data Buoy Center")
...
> https://docs.google.com/spreadsheets/d/1KVd_7IsOHM9Xe1PDIf47Q4LnQt4J6QFjS5UL1i_GzpA/edit?usp=sharing

> This dataset is a part of my independent capstone project studying the differences between fish communities in different types of habitat. I helped collect these data over the course of two summers along with several graduate and undergraduate students. This particular dataset looks at each drop or transect as a case, with previously calculated species richness and diversity measurements. This dataset can help answer these questions:
>
> -   Is there greater abundance and diversity of fishes in high relief versus low relief rocky substrate?
>
> -   Does species richness and diversity increase with depth?
>
> -   Are sampling methods (SCUBA or Lander) equivalent in capturing species richness and diversity?
